...
## 🆕 最近の更新
- **株式分割等の対応機能を追加**
  - 株式分割や株式併合などのコーポレートアクションに対応するため、調整済み価格（adjusted close）機能を追加
  - 「JPX400データ収集」実行時に、株式分割等が発生した場合、過去データを自動で調整済み価格に修正
  - 調整係数の変化を検出（1%以上の差）した場合、過去データの`open`, `high`, `low`, `close`, `adjusted_close`を自動更新
  - 「株式分割過去データ更新(debug用)」ボタンで、初回のみ全銘柄の過去データを一括更新可能
  - yfinanceのデータが間違っている場合に備え、「手動入力で修正」機能も追加
- **JPX400データ収集の処理速度向上（並列処理の導入）**
  - ThreadPoolExecutorを使用した並列処理により、400銘柄のデータ収集処理を約4分の1に短縮（4並列の場合）
  - デフォルトで4並列処理を実行（`max_workers`パラメータで調整可能）
  - 並列処理時は待機時間を最適化（0.1〜0.3秒）し、レート制限を回避しながら高速化
  - 順次処理モード（`max_workers=1`）も選択可能で、後方互換性を維持
- **スクリーニング結果一覧にRSI値を追加**
  - スクリーニング結果のテーブルに「RSI」列を追加し、各銘柄の現在のRSI（14日）を表示
  - 通常表示と履歴表示の両方に対応
  - データが不足している場合は「N/A」を表示
- **市況タブの銘柄一覧画面にRSI値を追加**
  - セクター別銘柄一覧画面に「RSI」列を追加
  - 各銘柄の現在のRSI（14日）を表示
  - RSI列のソート機能にも対応
- **市況タブの銘柄一覧画面の列幅調整**
  - セクター列と業種列の幅をそれぞれ20px縮小
  - その他の財務指標列の幅も最適化
...
- **株式分割等の自動対応**: データ収集時に、株式分割や株式併合などのコーポレートアクションが発生した場合、調整係数の変化を検出（1%以上の差）して過去データを自動で調整済み価格に修正します
  - 過去データの`open`, `high`, `low`, `close`, `adjusted_close`すべてを自動更新
  - 一度「株式分割過去データ更新(debug用)」ボタンを実行して初期設定を行えば、以後は「JPX400データ収集」ボタンだけで自動対応されます
...
#### 株式分割過去データ更新(debug用)
- **初回のみ使用**: 既存データに調整済み価格が設定されていない場合、または株式分割等が発生した過去データを修正する場合に使用します
- **使用方法**:
  1. 「株式分割過去データ更新(debug用)」ボタンをクリック
  2. 確認ダイアログで「はい」を選択
  3. 全銘柄の過去データが調整済み価格に更新されます（処理には時間がかかります）
- **重要**: 
  - **このボタンは初回のみ実行すればOKです**
  - 一度実行して株式分割の価格変更が完了したら、以後は「JPX400データ収集」ボタンを実行するだけで、株式分割等が発生した場合に自動で過去データが調整されます
  - 通常の運用では「JPX400データ収集」ボタンだけで十分です

#### 強制同期機能
- **1銘柄を強制同期**: 指定した銘柄・日付のデータをyfinanceから強制的に再取得してDBを更新します
- **指定日の全銘柄を強制同期**: 指定した日付の全銘柄のデータをyfinanceから強制的に再取得してDBを更新します
- **手動入力で修正**: yfinanceのデータが間違っている場合（例：8001の2026-01-13のデータ）、楽天などの信頼できる情報源から手動でデータを入力して修正できます
...
- **株式分割等の対応機能を追加**
  - 株式分割や株式併合などのコーポレートアクションに対応するため、調整済み価格（adjusted close）機能を追加
  - データベースに`adjusted_close`列を追加し、過去データも調整済み価格で保存
  - 「JPX400データ収集」実行時に、株式分割等が発生した場合、調整係数の変化を検出（1%以上の差）して過去データを自動で調整済み価格に修正
  - 過去データの`open`, `high`, `low`, `close`, `adjusted_close`すべてを自動更新
  - 「株式分割過去データ更新(debug用)」ボタンで、初回のみ全銘柄の過去データを一括更新可能
  - yfinanceのデータが間違っている場合に備え、「手動入力で修正」機能も追加
  - 強制同期機能を追加（1銘柄または指定日の全銘柄をyfinanceから強制的に再取得）
  - チャート表示、テクニカル分析（移動平均、MACD、KD）で調整済み価格を使用
...
### v1.16.0 (2026-01-04)
- **NC比率データ更新の定期実行機能を追加**
  - 毎日午前1時にNC比率データ更新を自動実行
  - Yahoo Financeから最新データを取得してキャッシュを更新し、NC比率を再計算
  - 自動実行スケジューラーに追加され、アプリ起動時に自動的に設定されます
- **スクリーニング履歴の表示を最適化**
  - パフォーマンス計算をバックグラウンドで実行するように改善
  - ウィンドウが即座に表示され、基本情報をすぐに確認可能
  - パフォーマンス計算はバックグラウンドで実行され、完了後に段階的に更新
  - これにより、ボタンを押してから画面が表示されるまでの時間を大幅に短縮
